...
(function($, window, document, undefined){
	...
	var Comment = {
		...
		buildForm_: function(comment_id, parent_id){
			...
			var product_id_field = $('<input/>');
			product_id_field.attr('type', 'hidden');
			product_id_field.attr('name', 'product_id');
			product_id_field.val($('#IdProduit').val());
			form_elem.append(product_id_field);
			...
			textarea.attr('placeHolder', 'Laisser un message...');
		...
		submitForm_: function(comment_id, form_data){
			...
			if(comment_id!=null) 	// form edit mode
				url_input = self.options.url_modify+'/'+comment_id;
